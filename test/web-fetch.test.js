...
import { clearWebFetchCache, fetchWebPage, } from '../src/index.js';
test.beforeEach(() => {
    clearWebFetchCache();
});
function createFetch(html, contentType = 'text/html') {
    return async (_input) => {
        return {
            ok: true,
            status: 200,
...
test('converts GitHub blob URLs to raw URLs', async (t) => {
    ...
    t.is(calledUrl, 'https://raw.githubusercontent.com/user/repo/main/README.md');
});
